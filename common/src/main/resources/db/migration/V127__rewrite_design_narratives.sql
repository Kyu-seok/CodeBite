-- V127: Phase 2C — narrative rewrites for the 16 design-interface problems.
--
-- After V125 (class/method renames) and V126 (fingerprint tightening), the
-- surface prose of the design problems still read as textbook data-structure
-- exercises. This migration replaces each description + constraints with a
-- domain-framed rewrite authored in batch, validated against the Phase-4
-- denylist, and manually spot-checked.
--
-- The class names, method signatures, sample test cases, and numeric bounds
-- are preserved verbatim — only the narrative framing changes. Korean
-- translations are refreshed in a separate migration (Phase 5).
--
