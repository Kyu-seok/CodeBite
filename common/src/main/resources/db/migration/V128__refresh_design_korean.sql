...
VALUES (
    ...
    'ko',
    $KO$클릭 집계 윈도우$KO$,
    $KO$마케팅 랜딩 페이지의 실시간 트래픽 대시보드에서 지난 몇 분 동안 페이지가 흡수한 클릭 수를 표시하려고 합니다. 요청은 서버 기준의 '일(day) 단위 초'로 태깅되어 도착하며, 대시보드는 항상 양 끝을 포함하는 5분 롤링 윈도우를 반영해야 합니다.
...
```
클릭이 한 건도 도착하기 전에 실행된 쿼리는 0을 반환하며, 이후에 기록된 클릭은 300초 시점의 윈도우 안에 여전히 포함됩니다.$KO$,
    $KO$- `1 <= timestamp <= 2 * 10^9`
- 전체 호출을 통틀어 전달되는 각 `timestamp`는 이전 값보다 크거나 같습니다.
- `record`와 `countSince`를 합쳐 총 호출 수는 최대 250회입니다.$KO$
);
...
VALUES (
    ...
    'ko',
    $KO$숏포스트 피드$KO$,
    $KO$각 계정이 짧은 글을 게시하고, 자신의 글과 현재 팔로우 중인 사용자들의 글을 합쳐 타임라인을 훑어볼 수 있는 경량 숏포스트 소셜 네트워크의 백엔드를 연결하고 있습니다. 타임라인은 항상 게시 시각 기준으로 최신순 정렬되어야 하며, 가장 최근 10개 항목으로 제한됩니다.
...
```
팔로우 관계를 끊는 즉시 계정 2의 기여분이 계정 1의 타임라인에서 빠집니다.$KO$,
    $KO$- `1 <= userId, followerId, followeeId <= 500`
- `0 <= messageId <= 8000`
- `publishPost`, `latestFeed`, `subscribe`, `unsubscribe`를 통틀어 최대 `20000`회의 호출이 발생합니다.$KO$
);
...
VALUES (
    ...
    'ko',
    $KO$체이닝 키-값 맵$KO$,
    $KO$정수 SKU 코드를 재고 수량에 매핑하는 소규모 부품 인벤토리 서비스를 구축하고 있습니다. 플랫폼 정책상 내장 딕셔너리 라이브러리를 가져올 수 없기 때문에, 룩업 테이블은 체이닝(또는 동등한 충돌 처리 기법)을 사용해 직접 조립해야 하지만 외부적으로는 편리한 set/lookup/erase 인터페이스를 제공해야 합니다.
...
```
한 번도 등록된 적 없는 SKU에 대한 조회와 삭제는 부작용 없이 처리됩니다.$KO$,
    $KO$- `0 <= key, value <= 800000`
- `set`, `lookup`, `erase`를 통틀어 최대 `8000`회의 호출이 발생합니다.$KO$
);
...
VALUES (
    ...
    'ko',
    $KO$K번째 최대 실시간 값$KO$,
    $KO$옥상 기상 연구소에서는 센서 뱅크로부터 정수 측정값들이 모니터링 서비스로 끊임없이 전달됩니다. 분석가들은 스트림이 늘어나는 동안 `k`번째로 큰 측정값을 지켜보고 싶어 합니다. 여기서 말하는 것은 `k`번째 *서로 다른* 값이 아니라 전체 정렬된 순서에서 `k`번째에 해당하는 값이므로, 중복된 값도 각자의 순위를 유지합니다.
...
추적기는 근무 시간 내내 반응성을 유지해야 합니다. 샘플마다 전체를 다시 정렬하는 단순한 방식으로는 금세 피드를 따라잡지 못합니다.$KO$,
    $KO$- `1 <= k <= 8000`
- `0 <= nums.length <= 8000`
- `-10^8 <= nums[i] <= 10^8`
- `-10^8 <= val <= 10^8`
- `record`는 최대 `8000`회까지 호출됩니다.
- `record`가 호출되는 시점에 합쳐진 스트림에는 최소 `k`개 이상의 측정값이 포함되어 있음이 보장됩니다.$KO$
);
...
VALUES (
    ...
    'ko',
    $KO$최솟값 추적 스택$KO$,
    $KO$운영팀은 프로덕션 트레이서에서 보내오는 서비스 지연 샘플을 실시간 로그에 쌓아두고 있습니다. 조사자들은 이 로그를 스택처럼 다룹니다. 측정값이 도착하면 뒤에 덧붙이고, 롤백 시에는 끝에서부터 팝(pop)해 나갑니다. 그리고 사이드카 대시보드는 현재 로그에 남아 있는 **최저** 지연 값을 매 순간 보여줘야 합니다. 트레이서가 지연되지 않도록 최저값 쿼리를 포함한 로그의 모든 연산은 상수 시간에 끝나야 합니다.
...
사이드카는 최소 한 번의 push가 이루어진 뒤에만 `pop`, `top`, `getMin`을 호출한다는 전제에 의존하므로, 이 세 연산에서는 로그가 비어있지 않다고 가정해도 됩니다.$KO$,
    $KO$- `-10^8 <= val <= 10^8`
- `pop`, `top`, `getMin`은 최소 한 개의 원소가 push된 뒤에만 호출됩니다.
- `push`, `pop`, `top`, `getMin`을 통틀어 최대 `20000`회의 호출이 발생합니다.$KO$
);
...
VALUES (
    ...
    'ko',
    $KO$접두사 인덱스$KO$,
    $KO$방대한 디렉터리 카탈로그를 색인하는 파일시스템 인덱서는 두 가지 질문에 빠르게 답할 수 있는 작은 자료구조가 필요합니다. "이 파일 이름을 정확히 본 적이 있는가?"와 "이 접두사로 시작하는 항목이 카탈로그에 존재하는가?"입니다. 이런 용도에는 글자 단위로 공유되는 트리가 잘 맞습니다. 각 글자마다 노드 하나씩만 따라가면 되므로 두 쿼리의 비용이 카탈로그 크기가 아니라 쿼리 길이에만 비례합니다.
...
`"apple"`을 삽입한 뒤 `search("apple")`은 해당 이름이 등록되어 있으므로 `true`입니다. `search("app")`은 `"app"`이 *접두사로만* 등장했을 뿐 독립된 이름으로 삽입된 적이 없으므로 `false`입니다. `startsWith("app")`은 `"apple"`이 그 세 글자로 시작하므로 `true`입니다. 이후 `"app"`이 정식으로 삽입되면 뒤따르는 `search("app")`은 `true`로 바뀝니다.$KO$,
    $KO$- `1 <= word.length, prefix.length <= 2000`
- `word`와 `prefix`는 영어 소문자로만 이루어져 있습니다.
- `insert`, `search`, `startsWith`를 통틀어 최대 `20000`회의 호출이 발생합니다.$KO$
);
...
VALUES (
    ...
    'ko',
    $KO$무작위 추첨 가방$KO$,
    $KO$래플 데스크는 추첨 자격이 있는 티켓 번호의 실시간 풀을 관리하며, 어느 시점에서든 그 중 하나를 균등하게 뽑아낼 수 있어야 합니다. 각 티켓 번호는 서로 다른 정수이며, 신청이 들어오거나 자격이 박탈되면서 풀의 크기는 커지거나 줄어듭니다. 매번 뽑을 때마다 현재 자격이 있는 모든 티켓은 동일한 확률로 뽑혀야 하고, 티켓이 아무리 많아져도 추가/제거/추첨의 기록 관리는 평균적으로 상수 시간에 유지되어야 합니다.
...
```
티켓 `1`과 티켓 `2`가 풀에 들어오고, 그 뒤 티켓 `1`이 자격을 잃습니다. 남아있는 자격 티켓은 `2`뿐이므로 추첨 결과는 반드시 `2`가 됩니다.$KO$,
    $KO$- `-10^8 <= val <= 10^8`
- `insert`, `remove`, `drawRandom`을 통틀어 최대 `2 * 10^5`회의 호출이 발생합니다.
- `drawRandom`이 호출되는 시점에 풀은 비어있지 않음이 보장됩니다.$KO$
);
...
VALUES (
    ...
    'ko',
    $KO$최근 사용 레지스트리$KO$,
    $KO$엣지 노드는 최근 참조된 레코드를 담는 용량 제한 레지스트리를 운영합니다. 각 레코드는 숫자 키로 식별되며 숫자 값을 함께 가집니다(재계산 비용이 비싼 응답이라고 생각하면 됩니다). 레지스트리가 가득 찬 상태에서 새로운 키가 들어오면, 가장 오래 건드려지지 않은 레코드를 밀어내 자리를 마련해야 합니다. 키에 대한 모든 접근은 — 새 쓰기든 성공적인 읽기든 — 해당 키를 "건드린 것"으로 간주되어 가장 최근에 사용된 항목으로 승격됩니다.
...
- `0 <= key <= 8000`
- `0 <= value <= 80000`
- `lookup`과 `store`를 통틀어 최대 `2 * 10^5`회의 호출이 발생합니다.$KO$
);
...
VALUES (
    ...
    'ko',
    $KO$고정 길이 링 버퍼$KO$,
    $KO$스트리밍 파이프라인이 원형으로 배치된 고정 크기 슬롯 블록을 재사용합니다. 데이터는 한쪽 끝으로 들어와 반대쪽 끝으로 빠져나가며, 쓰기 헤드가 블록의 끝에 도달하면 다시 시작 위치로 돌아오므로 저장 공간은 시작 시 정해진 용량을 절대 초과하지 않습니다. 파이프라인은 모든 슬롯이 차 있을 때의 쓰기를 거부하고, 버퍼가 비어 있을 때의 비우기를 무시하며, 앞뒤 값과 빈/가득 참 여부를 언제든 저렴한 비용으로 보고할 수 있어야 합니다.
...
```$KO$,
    $KO$- `1 <= k <= 1000`
- `0 <= value <= 1000`
- `push`, `pull`, `front`, `rear`, `isEmpty`, `isFull` 호출은 합쳐서 최대 `3000`회$KO$
);
...
VALUES (
    ...
    'ko',
    $KO$실시간 미디안 트래커$KO$,
    $KO$메트릭 수집기가 실시간 서비스에서 정수 샘플 스트림을 받아들이며, 어느 시점에서든 지금까지 관측한 모든 샘플의 미디안을 즉시 보고할 수 있어야 합니다. 샘플은 한 번에 하나씩 도착하며 제거되지 않습니다. 짝수 개의 샘플이 관측되었을 때 미디안은 가운데 두 값의 산술 평균으로 정의되어 부동소수점 값으로 반환되며, 홀수 개일 때는 가운데 값 자체가 (역시 부동소수점으로) 반환됩니다. 읽기와 쓰기가 자유롭게 섞여서 호출되므로, 아주 긴 세션에서도 두 연산 모두 저렴하게 동작해야 합니다.
...
- `median`은 첫 번째 `record` 이전에는 호출되지 않습니다$KO$
);
...
VALUES (
    ...
    'ko',
    $KO$스택으로 만든 큐$KO$,
    $KO$처리 파이프라인이 도착 순서(선입선출)대로 엄격히 처리해야 하는 작업들을 받지만, 이 환경에서 사용할 수 있는 기본 저장 자료구조는 후입선출 스택뿐입니다. 여러분의 과제는 내부적으로 정확히 두 개의 스택만 백업 저장소로 쓰면서 FIFO 인터페이스를 노출하는 것입니다. 배열, 리스트, 덱, 그 밖의 어떤 컨테이너도 허용되지 않습니다. 두 내부 스택에 대해서는 표준 스택 연산(상단에 push, 상단에서 pop, 상단 peek, 비어 있는지 검사)만 사용할 수 있습니다.
...
- `pop`과 `peek`은 파이프라인이 비어 있지 않을 때만 호출됩니다$KO$
);
...
VALUES (
    ...
    'ko',
    $KO$버전 관리되는 셀 배열$KO$,
    $KO$구성 관리 서비스가 운영자가 값을 덮어쓰는 고정 길이의 숫자 셀 행을 관리하며, 주기적으로 현재 행을 감사용 불변 스냅샷으로 고정합니다. 모든 스냅샷은 `0`부터 시작해 차례대로 다음 번호를 부여받으며, 이후 읽기 요청은 그 스냅샷이 찍히던 순간 특정 셀이 어떤 값을 가지고 있었는지를 정확히 복원할 수 있어야 합니다 — 그 뒤로 아무리 많은 편집이 있었더라도 말이죠. 서비스 생성 시 모든 셀은 `0`으로 시작합니다.
...
- `set`, `commit`, `readAt` 호출은 합쳐서 최대 `50000`회$KO$
);
...
VALUES (
    ...
    'ko',
    $KO$문자열 리스트 코덱$KO$,
    $KO$두 서비스 사이에 메시지 프레이밍 계층이 자리하고 있는데, 이 두 서비스는 소켓을 통해 오직 하나의 원시 문자열만 주고받을 수 있습니다. 송신 측은 임의의 문자열 조각들로 이루어진 리스트를 수신 측에 전달해야 하고, 수신 측은 그 리스트를 바이트 단위로 동일하게 복원해야 합니다. 여러분이 할 일은 양쪽 끝에 공통으로 들어가는 프레이밍 코덱을 만드는 것입니다.
...
이 코덱은 반드시 **무상태(stateless)** 여야 합니다. `unpack`이 필요로 하는 모든 정보는 `pack`이 만들어 낸 문자열 안에만 존재해야 하며, 공유 카운터나 외부 메타데이터, 호출 간에 유지되는 인스턴스 필드 같은 것에 의존해서는 안 됩니다. 리스트가 비어 있거나, `hello`나 `world` 같은 빈 조각들만 담고 있거나, 프레이밍 마커처럼 보이는 문자들로만 이루어진 조각을 포함하더라도, `pack` → `unpack`을 거친 왕복은 입력 리스트를 정확히 재현해야 합니다.$KO$,
    $KO$- `0 <= strs.length <= 200`
- `0 <= strs[i].length <= 200`
- 각 `strs[i]`는 코덱이 구분자나 이스케이프로 사용하는 문자를 포함해 어떤 유니코드 코드 포인트든 담을 수 있습니다.$KO$
);
...
VALUES (
    ...
    'ko',
    $KO$타임스탬프 값 저장소$KO$,
    $KO$메트릭 플랫폼이 장비 계측으로부터 이름이 붙은 신호를 받아들입니다. 각 신호는 문자열 `key`(예: 장치 식별자나 메트릭 이름)로 구분되며, 장치가 보고할 때마다 해당 키에 대해 새로운 `value`가 기록됩니다. 같은 키에 대한 보고는 타임스탬프가 엄격하게 증가하는 순서로 도착합니다. 나중에 분석가들은 특정 키와 타임스탬프 `t`에 대해 "이 신호는 `t` 시점 기준으로 어떤 값이었나?"라고 묻는데, 이는 `t`보다 미래가 아닌 타임스탬프 중 가장 최근에 기록된 값을 의미합니다.
...
쿼리는 기록된 두 타임스탬프 사이의 시점을 물어볼 수도 있습니다. 그 경우에는 그보다 앞선 가장 최근 쓰기 시점의 값이 유효했던 값이므로 `readAt`은 그 값을 반환합니다. 쿼리는 가장 최근 쓰기보다 더 뒤의 타임스탬프에 대해서도 도착할 수 있으며, 이때는 그 가장 최근 값을 드러내야 합니다.$KO$,
    $KO$- `1 <= key.length, value.length <= 100`
- `1 <= timestamp <= 10^7`
- `write`에 전달되는 타임스탬프는 각 `key`마다 엄격하게 증가합니다.
- `write`와 `readAt` 호출의 총합은 최대 `2 * 10^5`입니다.$KO$
);
...
VALUES (
    ...
    'ko',
    $KO$이진 트리 직렬화기$KO$,
    $KO$계층적 문서 워크플로는 이진 트리 형태의 아티팩트(각 노드가 선택적인 왼쪽/오른쪽 자식과 정수 라벨을 가지는 콘텐츠 개요라고 생각하면 됩니다)를 문자열만 주고받을 수 있는 서비스들 사이에서 전달해야 합니다. 여러분의 과제는 그 와이어 포맷의 양쪽을 작성하는 것입니다. 한쪽은 실제 트리를 하나의 문자열로 평탄화하고, 다른 쪽은 그 문자열로부터 동일한 트리를 재구성합니다.
...
문자열 밖의 어떤 상태에도 의존할 수 없습니다 — 직렬화기와 역직렬화기는 무상태여야 하며, 두 개의 서로 다른 프로세스가 오직 `pack`/`unpack`만으로 트리를 교환할 수 있어야 합니다.$KO$,
...
VALUES (
    ...
    'ko',
    $KO$와일드카드 단어 사전$KO$,
    $KO$맞춤법 검사 서브시스템이 알려진 소문자 단어들의 사전을 점점 늘려가며 유지합니다. 작성자는 언제든 새 단어를 등록할 수 있고, 검토자는 간단한 패턴 언어로 사전에 질의합니다. 정확한 글자는 그 글자에 그대로 대응해야 하고, `.`은 임의의 단일 글자와 매칭됩니다. 저장된 단어들 중 최소 하나가 패턴과 같은 길이에서 위 규칙에 따라 글자별로 매칭되면 그 패턴은 만족된 것입니다.
...
쿼리의 `word`는 소문자 알파벳과 와일드카드 문자 `.`만 포함합니다. 와일드카드가 아닌 문자는 대응되는 자리와 정확히 일치해야 하고, `.`은 임의의 소문자 하나를 대신할 수 있지만 여전히 저장된 단어의 한 글자를 정확히 소비합니다. 쿼리는 사전을 변경하지 않고 조회만 하며, 등록과 쿼리는 임의 순서로 섞여 호출될 수 있습니다.$KO$,
    $KO$- `1 <= word.length <= 25`
- `enroll`의 `word`는 소문자 영어 알파벳으로만 이루어집니다.
- `match`의 `word`는 소문자 영어 알파벳과/또는 와일드카드 문자 `.`로 이루어집니다.
- `enroll`과 `match` 호출은 합쳐서 최대 `8000`회입니다.$KO$
);
